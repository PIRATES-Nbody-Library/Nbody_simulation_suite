...
c Date:    8/25/94
c Last revision: 05/27/26 Thomas A. Stuber, based on changes from
c                         Virginie Faramaz-Gorka and Hervé Beust; from
c                         now on tracked on GitHub
	...
	character*80 diro,dirs,gname,outfile_full
...
c...    Do the initial io write
     ...
     &      xht,yht,zht,vxht,vyht,vzht,istat,outfile_full,iub,fopenstat)
        endif
c	write(*,*) 'CHECKPOINT'
...
c If it is time, do a dump
 ...
 998         format(' Time = ',1p1e12.5,': fraction done = ',0pf5.3,
     &            ': Number of active tp =',i4)
	     call io_dump_pl(trim(diro)//'/dump_pl.dat',nbod,mass,xh,yh,
