...
c...  Now do the interpolation for intermediate steps
      if(icflg.eq.-1) then
         call rmvs2_interp(nbod,xbeg,ybeg,zbeg,vxbeg,vybeg,vzbeg,
     &        xend,yend,zend,vxend,vyend,vzend,dt,mass(1),
     &        xtmp,ytmp,ztmp,xtmpo,ytmpo,ztmpo)
      else
         call rmvs2_interp_o(nbod,xbeg,ybeg,zbeg,vxbeg,vybeg,vzbeg,
     &        xend,yend,zend,vxend,vyend,vzend,dt,mass(1),
     &        xtmpo,ytmpo,ztmpo)
      endif

c...  first do the outer region integration, save the planet position
      call rmvs2_step_out(i1st,nbod,ntp,mass,j2rp2,j4rp4,
